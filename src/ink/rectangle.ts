...
export class Rectangle {
    ...
    public intersection(other: Rectangle): Rectangle {
        ...
        const blx = Math.max(this.x, other.x);
        const bly = Math.max(this.y, other.y);

        // gives top-right point
        // of intersection rectangle
        const tpx = Math.min(this.x + this.width, other.x + other.width);
        const tpy = Math.min(this.y + this.height, other.y + other.height);
        ...
        if (blx > tpx || bly > tpy) {
            ...
            return new Rectangle(blx, bly, tpx - blx, tpy - bly);
        }
    }

    // FIX: dup (within)
    public containsPoint(p: IPoint) {
        return (p.x >= this.x) && (p.y >= this.y) && (p.x <= (this.x + this.width)) &&
            (p.y <= (this.y + this.height));
    }

    public overlaps(other: Rectangle): boolean {
        const r1x = this.x + this.width;
        const r2x = other.x + other.width;
        const r1y = this.y + this.height;
        const r2y = other.y + other.height;

        if ((this.x >= r2x) || (other.x >= r1x)) {
            return false;
        }

        if ((this.y <= r2y) || (other.y <= r1y)) {
            return false;
        }
